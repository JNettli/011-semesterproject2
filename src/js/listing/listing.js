import { headerKey, singleListing, profileRequest } from '../constants.js'
        ...
        if(Number(bidInput.value) <= Number(localStorage.getItem('credits'))) {
            if(Number(bidInput.value) > Number(listing._count.bids) || Number(bidInput.value) > 0) {
                ...
                if(apires.ok) {
                    ...
                    if(apidata.message) {
                        console(apidata.message)
                    } else {
                        console.error("Wow this done goofed")
                    }
                }
            }
            makeBid();
        } else {
            alert('You need to put a number higher than the lowest bid!');
            }
        } else {
            alert('You cannot bid more than you have credits!')
        }
    });
