...
async function getSingleProfile() {
	...
	const listingsOfProfile = await fetch(
		`${profileRequest}${userProfileId}/listings?limit=${itemsPerPage}&_bids=true&page=${currentPage}&sort=${sortBy}&sortOrder=${ascDesc}&_active=${endedBool}`,
		{
			headers: {
				Authorization: `Bearer ${localStorage.getItem('token')}`,
				'X-Noroff-API-Key': headerKey,
	...
	filterOptionBox.classList.add('flex', 'flex-col', 'gap-4', 'self-center', 'md:flex-row');
	...
	if (listings.length > 0) {
		listings.forEach((listing) => {
			...
			listingDiv.classList.add('bg-white', 'dark:bg-gray-800', 'p-6', 'rounded', 'flex', 'flex-col', 'items-center', 'md:flex-row', 'gap-8', 'mb-4', 'w-full', 'relative', 'z-0'
			);
			listingBox.appendChild(listingDiv);

			const listingImageAnchor = document.createElement('a');
			listingImageAnchor.href = `/listing/?listingId=${listing.id}`;
			listingImageAnchor.classList.add('flex-shrink-0');
