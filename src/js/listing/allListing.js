  ...
  async function getSearchedListings() {
    ...
    data.data.forEach((listing) => {
      ...
      listingEnds.innerText =
        Math.floor(
          (Date.parse(listing.endsAt) - Date.now()) / 1000 / 60 / 60 / 24
        ) +
        " days " +
        Math.floor(
          ((Date.parse(listing.endsAt) - Date.now()) / 1000 / 60 / 60) % 24
        ...
        " hours " +
        Math.floor(
          ((Date.parse(listing.endsAt) - Date.now()) / 1000 / 60) % 60
        ) +
        " minutes remaining";
      listingEnds.classList.add(
        "text-lg",
        "font-semibold",
        "text-black",
        "dark:text-white"
